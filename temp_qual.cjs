...
exports.calculateQualification = calculateQualification;
exports.assignKnockoutMatches = assignKnockoutMatches;
function calculateQualification(standings) {
    const groupWinners = {};
            ...
            if (isGroupFinished) {
                ...
                if (guaranteedFirst) {
                    ...
                    if (team.maxPoints < currentFirstPoints) {
                        ...
                        status = 'QUALIFIED'; // Secured Top 2, but exact seed TBD
                    }
                }
 ...
 */
function resolveLabel(label, qual, availableThirds) {
    if (!label)
        return null;
...
function assignKnockoutMatches(games, qual) {
